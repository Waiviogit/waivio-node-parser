...
describe( 'Wobject model', async () => {
    describe( 'On addVote', async () => {
        let result, data, voter, wobject, appendObject;
        beforeEach( async () => {
    ...
    describe( 'On updateField', async () => {
        let result, key, value, field, resultCreateAppend;
        key = 'weight';
        value = 550;
        beforeEach( async () => {
            await dropDatabase();
    ...
    describe( 'On update', async () => {
        let condition, updateData, wobject;
        beforeEach( async () => {
            await dropDatabase();
        ...
        it( 'shouldn\'t update wobject with incorrect data ', async () => {
            ...
            let findPost = await WObject.findOne( { author_permlink: wobject.author_permlink } );
    ...
    describe( 'On increaseFieldWeight', async () => {
        let result, resultCreateAppend, field, weight;
        beforeEach( async () => {
            await dropDatabase();
            resultCreateAppend = await AppendObject.Create();
            field = resultCreateAppend.appendObject;
        ...
        it( 'should return true with correct input', async () => {
            result = await WobjModel.increaseFieldWeight( { author: field.author, permlink: field
                .permlink, author_permlink: resultCreateAppend.root_wobj, weight: weight } );
            expect( result.result ).is.true;
        } );
    ...
    describe( 'On removeVote', async () => {
        let result, data, field, voter, resultCreateAppend;
        beforeEach( async () => {
            await dropDatabase();
            resultCreateAppend = await AppendObject.Create();
            field = resultCreateAppend.appendObject;
