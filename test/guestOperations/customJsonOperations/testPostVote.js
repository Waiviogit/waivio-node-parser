const { expect, sinon, faker, postsUtil, Post, Comment, votePostHelper } = require( '../../testHelper' );
const { guestVote } = require( '../../../utilities/guestOperations/customJsonOperations' );
const { UserFactory, ObjectFactory, PostFactory, CommentFactory } = require( '../../factories' );
            ...
            describe( 'when vote on steem COMMENT', async () => {
                ...
                describe( 'on Valid input json', async () => {
                    describe( 'when comment exist in STEEM and DB', async () => {
                        let validJson, voter, comment, mockSteemComment, vote;
                        beforeEach( async () => {
