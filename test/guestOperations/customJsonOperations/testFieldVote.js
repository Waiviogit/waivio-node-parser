const { expect, sinon, faker, WObject, UserWobjects, voteFieldHelper } = require( '../../testHelper' );
...
describe( 'customJsonOperations', async () => {
    ...
    describe( 'on guestVote', async () => {
        describe( 'on voteOnField', async () => {
            let wobject, voter, field, validJson, voterUserWobj, creatorUserWobj;
            beforeEach( async () => {
            ...
            describe( 'on valid input json', async () => {
                let upd_wobject, upd_field, updVoterUserWobj, updCreatorUserWobj;
                beforeEach( async () => {
