...
const updatePostWithExpForecast = async ({parent_author, parent_permlink, author, exp_forecast}) => {
    const isValid = await validateExpForecast({parent_permlink, parent_author, author});
    if (isValid) {
        const {result, error} = await Post.update({author: parent_author, permlink: parent_permlink, exp_forecast});
        if (result)
            return true;
    }
    return false;
...
const validateExpForecast = async ({parent_author, parent_permlink, author, exp_forecast}) => {
    const {post, err} = await Post.findOne({author: parent_author, permlink: parent_permlink});
