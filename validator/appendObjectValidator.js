...
const validateSpecifiedFields = async ( data ) => {
    switch ( _.get( data, 'field.name' ) ) {
        case 'parent' :
            ...
            if( !parentWobject )
                throw new Error( `Can't append parent ${data.field.body}, wobject should exist` );
            break;
        case 'newsFilter' :
            let newsFilter;
            try{
            ...
            if( !validateNewsFilter( newsFilter ) )
                throw new Error( `Can't append newsFilter ${data.field.body}, not valid data` );
            break;
        case 'map' :
            let map;
            try{
        ...
        case 'tagCategory' :
            ...
            if( !_.get( data, 'field.id' ) )
                throw new Error( `Can't append tagCategory ${data.field.body}, "id" is required` );
            // tagCategory must to be unique by id
            const { wobject: tagCategoryWobj } = await Wobj.getOne( { author_permlink: data.author_permlink } );
        ...
        case 'categoryItem' :
            // "id" field is required
            if( !_.get( data, 'field.id' ) )
                throw new Error( `Can't append categoryItem ${data.field.body}, "id" is required` );
            ...
            if( !existTag || existTag.object_type !== 'hashtag' )
                throw new Error( `Can't append categoryItem ${data.field.body}, Hashtag not valid!` );

            const { wobject: categoryItemWobj } = await Wobj.getOne( { author_permlink: data.author_permlink } );
            const parentCategory = _.chain( categoryItemWobj ).get( 'fields', [] )
                .find( { name: 'tagCategory', id: data.field.id } ).value();
            if( !parentCategory )
                throw new Error( `Can't append categoryItem ${data.field.body}, "tagCategory" with the same "id" doesn't exist` );
            const existItem = _
